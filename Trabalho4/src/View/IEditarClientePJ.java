...
public class IEditarClientePJ extends JFrame implements ActionListener {
	...
	public IEditarClientePJ(DAOClientes cliente, PessoaJuridica c) {
		...
		fieldNome = new JTextField(pj.getNome());
		...
		comboRenda.setSelectedItem(pj.getRenda());
		add(comboRenda);
		...
		fieldEndereco = new JTextField(pj.getEndereco());
		...
		fieldBairro = new JTextField(pj.getBairro());
		...
		try {
			ffieldCep = new JFormattedTextField(pj.getCep());
		...
		fieldCidade = new JTextField(pj.getCidade());
		...
		comboUf.setSelectedItem(pj.getUf());
		add(comboUf);
		...
		try {
			ffieldTelefone = new JFormattedTextField(pj.getTelefone());
		...
		try {
			ffieldCnpj = new JFormattedTextField(pj.getCnpj());
			ffieldCnpj.setBounds(70, 115, 95, 25);
			ffieldCnpj.setEditable(false);
			add(ffieldCnpj);
		} catch (Exception e) {
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (comando.equals("Cadastrar")) {
			...
			cliente.editarClientePJ(nome, renda, endereco, bairro, cep, cidade, uf, telefone, situacao, cnpj);
			clean();
			this.dispose();
		}
